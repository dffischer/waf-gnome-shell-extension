...
also installed and compiled when the glib2 tool is loaded.
    ...
    def build(bld):
        bld(features="gse",
                ...
                source="additional sources",  # scanned for imports
                data="additional data files")  # not scanned

Imports of the form 'const <something> = Me.imports.<import>;' are
automatically recursively detected in the entrypoint javascript files and
sources. An alternative pattern to find these can be injected using the
set_inclusion functions, either globally on the context or on a dedicated task
generator.
...
The scanning function is also available from the task
generator. Note, however, that this will not simply work with files that first
have to be generated by other tasks. The scanning mechanism running on the
files fiven in the source parameter is postponed until these are available.
...
class gse_producer(Task):
    ...
    def sig_explicit_deps(self):
        try:
            ...
            self.inputs = self.inputs[0:2] + self.inputs[3:]
            return super().sig_explicit_deps()
